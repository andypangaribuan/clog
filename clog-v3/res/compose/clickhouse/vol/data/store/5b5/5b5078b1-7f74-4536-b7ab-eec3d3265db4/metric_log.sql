ATTACH TABLE _ UUID '963e1731-a3b1-4528-9f96-8b32d652fa97'
(
    ...
    `ProfileEvent_ASTFuzzerSkippedBackupRestore` UInt64 COMMENT 'Number of fuzzed BACKUP/RESTORE queries the server-side AST fuzzer skipped instead of executing.',
    `ProfileEvent_ASTFuzzerSkippedReplicatedDDLInternal` UInt64 COMMENT 'Number of times the server-side AST fuzzer skipped fuzzing because an internal replicated-database DDL execution (a live ZooKeeperMetadataTransaction) was in flight on the context.',
    ...
    `ProfileEvent_UniqueKeyBitmapLoadMicroseconds` UInt64 COMMENT 'Time spent reading and deserializing UNIQUE KEY delete-bitmap sidecar files on a DeleteBitmapCache miss.',
    `ProfileEvent_UniqueKeyBitmapUpdates` UInt64 COMMENT 'Number of UNIQUE KEY delete-bitmap sidecar files written (one per supersession / DELETE commit).',
    ...
    `ProfileEvent_PaimonMetadataFilesCacheHits` UInt64 COMMENT 'Number of times paimon metadata files have been found in the cache.',
    `ProfileEvent_PaimonMetadataFilesCacheMisses` UInt64 COMMENT 'Number of times paimon metadata files have not been found in the paimon metadata cache and had to be read from (remote) disk.',
    `ProfileEvent_PaimonMetadataFilesCacheWeightLost` UInt64 COMMENT 'Approximate number of bytes evicted from the paimon metadata cache.',
    ...
    `ProfileEvent_IcebergMetadataUpdateMicroseconds` UInt64 COMMENT 'Total time spent updating iceberg storage metadata from state (buildStorageMetadataFromState).',
    ...
    `ProfileEvent_TextIndexPostingsCacheMisses` UInt64 COMMENT 'Number of times a text index posting list has not been found in the cache.',
    ...
    `ProfileEvent_TextIndexPositionsDecodeMicroseconds` UInt64 COMMENT 'Total time spent decoding text index position lists (.pos) for phrase search.',
    `ProfileEvent_TextIndexPhraseMatchMicroseconds` UInt64 COMMENT 'Total time spent in the roaringish phrase-match intersection.',
    ...
    `ProfileEvent_TextIndexLazyPackedBlocksDecoded` UInt64 COMMENT 'Number of packed blocks decoded in lazy posting list mode.',
    `ProfileEvent_TextIndexLazyAdvanceCount` UInt64 COMMENT 'Number of advance operations performed in lazy posting list mode.',
    `ProfileEvent_TextIndexLazySegmentsPrepared` UInt64 COMMENT 'Number of segments prepared (read from disk or cached) in lazy posting list mode.',
    `ProfileEvent_TextIndexLazySegmentsBuilt` UInt64 COMMENT 'Number of segments actually read and decoded (cache misses) in lazy posting list mode.',
    `ProfileEvent_TextIndexLazyBruteForceIntersections` UInt64 COMMENT 'Number of brute-force intersections performed in lazy posting list mode.',
    `ProfileEvent_TextIndexLazyLeapfrogIntersections` UInt64 COMMENT 'Number of leapfrog intersections performed in lazy posting list mode.',
    `ProfileEvent_TextIndexLazySegmentsSkippedDense` UInt64 COMMENT 'Number of fully-dense segments padded as a whole (memset for OR, increment for AND) instead of decoding blocks, in lazy posting list mode.',
    `ProfileEvent_TextIndexLazySegmentsSkippedResolved` UInt64 COMMENT 'Number of segments skipped because the output region was already resolved (all-ones for OR, all-zeros for AND) in lazy posting list mode.',
    `ProfileEvent_TextIndexLazyBlocksSkippedResolved` UInt64 COMMENT 'Number of packed blocks skipped because the output region was already resolved (all-ones for OR, all-zeros for AND) in lazy posting list mode.',
    ...
    `ProfileEvent_TextIndexGenericExclusionSearchAlgorithm` UInt64 COMMENT 'Number of times the generic exclusion search algorithm is used over the text index.',
    `ProfileEvent_TextIndexGenericExclusionSearchStepLimitReached` UInt64 COMMENT 'Number of times the generic exclusion search over the text index reached merge_tree_generic_exclusion_search_max_steps and accepted the remaining mark ranges without further splitting.',
    ...
    `ProfileEvent_EncryptionHeaderCacheHits` UInt64 COMMENT 'Number of times encryption header bytes were found in the encryption header cache, so the source read of the header was skipped.',
    `ProfileEvent_EncryptionHeaderCacheMisses` UInt64 COMMENT 'Number of times encryption header bytes were not found in the encryption header cache, so they were read from the source.',
    ...
    `ProfileEvent_ReaderExecutorSourceRequests` UInt64 COMMENT 'Number of source-side requests opened by ReaderExecutor (excludes live-buffer reuses).',
    `ProfileEvent_ReaderExecutorBytesFromSource` UInt64 COMMENT 'Physical bytes ReaderExecutor issued to the source after missing all cache tiers (foreground plus background prefetch, including a prefetch\'s bytes wasted by a later discard); not consumer-served bytes - see ReaderExecutorRequestedBytes.',
    `ProfileEvent_ReaderExecutorRequestedBytes` UInt64 COMMENT 'Useful bytes ReaderExecutor delivered to read requests (the requested window payload, excluding over-read and cache write-back). Denominator for the modeled cost-per-byte KPI (ReaderExecutorModeledCostMicroseconds / ReaderExecutorRequestedBytes).',
    `ProfileEvent_ReaderExecutorCacheGetRequests` UInt64 COMMENT 'Number of ICacheHandle::get invocations in ReaderExecutor. Zero until the ReaderExecutor cache tiers are introduced.',
    `ProfileEvent_ReaderExecutorCachePopulateRequests` UInt64 COMMENT 'Number of ICacheHandle::put invocations in ReaderExecutor. Zero until the ReaderExecutor cache tiers are introduced.',
    `ProfileEvent_ReaderExecutorIncompleteConnections` UInt64 COMMENT 'Number of source connections ReaderExecutor dropped before draining them to their right bound; not pool-reusable, forcing a re-establishment. Zero until ReaderExecutor live source-buffer reuse is introduced.',
    `ProfileEvent_ReaderExecutorWorkMicroseconds` UInt64 COMMENT 'Total wall-clock time spent inside ReaderExecutor::readNextWindow (opening, seeking and reading the served window). Direct contributor to query read latency.',
    `ProfileEvent_ReaderExecutorDecryptMicroseconds` UInt64 COMMENT 'Time ReaderExecutor spent decrypting served payload in place (CTR, position-addressable). Zero for unencrypted reads and in builds without SSL.',
    `ProfileEvent_ReaderExecutorModeledCostMicroseconds` UInt64 COMMENT 'Modeled I/O cost of ReaderExecutor reads: a synthetic proxy KPI for read-path optimality, NOT measured latency. Weighted sum of the counters above with heuristic S3 weights: 30ms per source request + 5ms per incomplete connection + 20ms per MiB transferred from source (useful payload plus over-read) + 0.1ms per cache put + 0.05ms per cache get. Divide by ReaderExecutorRequestedBytes for a load-independent cost-per-byte. Experimental, tracks the experimental ReaderExecutor.',
    `ProfileEvent_ReaderExecutorLongConnectionOpened` UInt64 COMMENT 'Number of long source connections opened by ReaderExecutor for sequential read optimization.',
    `ProfileEvent_ReaderExecutorLongConnectionHits` UInt64 COMMENT 'Number of windows ReaderExecutor served by reading from an already-open long source connection.',
    `ProfileEvent_ReaderExecutorLongConnectionFallbacks` UInt64 COMMENT 'Number of times ReaderExecutor wanted a long connection but fell back to a one-shot read because no slot was available.',
    `ProfileEvent_ReaderExecutorLongConnectionBytes` UInt64 COMMENT 'Total bytes read through long source connections.',
    ...
    `ProfileEvent_ZooKeeperWatchCallbackDurationMicroseconds` UInt64 COMMENT 'Total time spent inside ZooKeeper watch callbacks (network thread).',
    `ProfileEvent_ZooKeeperWatchCallbackErrors` UInt64 COMMENT 'Number of exceptions thrown from ZooKeeper watch callbacks.',
    `ProfileEvent_ZooKeeperWatchTriggeredOther` UInt64 COMMENT 'Number of watch notifications dispatched to uncategorized callbacks.',
    `ProfileEvent_ZooKeeperWatchTriggeredReplicatedMergeTreeLog` UInt64 COMMENT 'Number of watch notifications dispatched to ReplicatedMergeTree log entries.',
    `ProfileEvent_ZooKeeperWatchTriggeredReplicatedMergeTreeMutations` UInt64 COMMENT 'Number of watch notifications dispatched to ReplicatedMergeTree mutations.',
    `ProfileEvent_ZooKeeperWatchTriggeredReplicatedMergeTreeLeaderElection` UInt64 COMMENT 'Number of watch notifications dispatched to ReplicatedMergeTree leader election / replica locks.',
    `ProfileEvent_ZooKeeperWatchTriggeredReplicatedMergeTreeReplicaSync` UInt64 COMMENT 'Number of watch notifications dispatched while waiting for replicas to process log entries (SYNC REPLICA, ALTER, mutations).',
    `ProfileEvent_ZooKeeperWatchTriggeredDistributedDDL` UInt64 COMMENT 'Number of watch notifications dispatched to DDLWorker queue.',
    `ProfileEvent_ZooKeeperWatchTriggeredReplicatedAccessControl` UInt64 COMMENT 'Number of watch notifications dispatched to ReplicatedAccessStorage.',
    `ProfileEvent_ZooKeeperWatchTriggeredUserDefinedSQLObjects` UInt64 COMMENT 'Number of watch notifications dispatched to UserDefinedSQLObjectsZooKeeperStorage.',
    `ProfileEvent_ZooKeeperWatchTriggeredWorkloadEntity` UInt64 COMMENT 'Number of watch notifications dispatched to WorkloadEntityKeeperStorage.',
    `ProfileEvent_ZooKeeperWatchTriggeredBackupCoordination` UInt64 COMMENT 'Number of watch notifications dispatched to backup / restore coordination.',
    `ProfileEvent_ZooKeeperWatchTriggeredObjectStorageQueue` UInt64 COMMENT 'Number of watch notifications dispatched to ObjectStorageQueue / S3Queue.',
    `ProfileEvent_ZooKeeperWatchTriggeredClusterDiscovery` UInt64 COMMENT 'Number of watch notifications dispatched to ClusterDiscovery.',
    `ProfileEvent_ZooKeeperWatchTriggeredMaterializedViewRefresh` UInt64 COMMENT 'Number of watch notifications dispatched to MaterializedView refresh coordination.',
    ...
    `ProfileEvent_QueryParseMicroseconds` UInt64 COMMENT 'Total time spent parsing the query text into an AST.',
    `ProfileEvent_QueryAnalysisMicroseconds` UInt64 COMMENT 'Total time spent building the query tree and running analyzer passes (resolves identifiers, types, expressions).',
    `ProfileEvent_QueryPlanBuildMicroseconds` UInt64 COMMENT 'Total time spent building the QueryPlan from the analyzed query tree.',
    `ProfileEvent_QueryPipelineBuildMicroseconds` UInt64 COMMENT 'Total time spent constructing the QueryPipeline from the optimized QueryPlan.',
    ...
    `ProfileEvent_UniqueKeyIndexCacheLookupMicroseconds` UInt64 COMMENT 'Wall-clock time inside `UniqueKeyIndexCache::Lookup` + `UniqueKeyIndexCache::Insert` (ClickHouse-side `CacheBase` adapter for the RocksDB block cache).',
    `ProfileEvent_UniqueKeyIndexCacheHits` UInt64 COMMENT 'Number of times an entry has been found in the UNIQUE KEY index cache, so we didn\'t have to load an SST block.',
    `ProfileEvent_UniqueKeyIndexCacheMisses` UInt64 COMMENT 'Number of times an entry has not been found in the UNIQUE KEY index cache, so we had to load an SST block from disk.',
    `ProfileEvent_UniqueKeySSTWriteMicroseconds` UInt64 COMMENT 'Total wall-clock time spent inside an `SSTIndexWriter` lifetime — covers SST `Open`, every `addEncoded` Put, and `Finish` + copy-via-`writeFile` in `finalizeToStorage`. Excludes work the static helpers do before constructing the writer (encode + non-prefix-path sort). Emitted once per writer.',
    ...
    `ProfileEvent_RowPolicyCacheRecalculations` UInt64 COMMENT 'Number of times the row policy cache re-mixed filters for all live enabled sets. Coalesced to once per access entity notification batch.',
    `ProfileEvent_RowPolicyCacheRecalculationMicroseconds` UInt64 COMMENT 'Total time spent re-mixing row policy filters for all live enabled sets, off the RowPolicyCache mutex (so it does not block the ContextAccess build path).',
    `ProfileEvent_RoleCacheRecalculations` UInt64 COMMENT 'Number of times the role cache recalculated all live enabled sets. Coalesced to once per access entity notification batch.',
    `ProfileEvent_RoleCacheRecalculationMicroseconds` UInt64 COMMENT 'Total time spent recalculating all live enabled role sets (held under the cache mutex that the ContextAccess build path also takes).',
    `ProfileEvent_SettingsProfileCacheRecalculations` UInt64 COMMENT 'Number of times the settings profile cache re-merged settings and constraints for all live enabled sets. Coalesced to once per access entity notification batch.',
    `ProfileEvent_SettingsProfileCacheRecalculationMicroseconds` UInt64 COMMENT 'Total time spent re-merging settings and constraints for all live enabled sets (held under the cache mutex that the ContextAccess build path also takes).',
    `ProfileEvent_QuotaCacheRecalculations` UInt64 COMMENT 'Number of times the quota cache re-chose quotas for all live enabled sets. Coalesced to once per access entity notification batch.',
    `ProfileEvent_QuotaCacheRecalculationMicroseconds` UInt64 COMMENT 'Total time spent re-choosing quotas for all live enabled sets (held under the cache mutex that the ContextAccess build path also takes).',
    `ProfileEvent_FileProgressCallbackInvocations` UInt64 COMMENT 'Number of times the per-query `FileProgressCallback` was invoked (counts each per-file `FileProgress` event delivered to the client over native TCP or via `clickhouse-local`).',
    ...
    `ProfileEvent_GlobalMemoryLimitExceeded` UInt64 COMMENT 'Number of times the global memory limit was exceeded.',
    ...
    `ProfileEvent_CachedReadBufferCacheWriteStopped` UInt64 COMMENT 'Number of times writing a file segment into the filesystem cache was stopped mid-download (space reservation or the cache write failed), after which the read continues bypassing the cache',
    ...
    `ProfileEvent_CachedWriteBufferCacheWriteStopped` UInt64 COMMENT 'Number of times write-through caching was stopped (space reservation or the cache write failed, or a covering segment was being evicted), after which the write continues without populating the cache',
    ...
    `ProfileEvent_FilesystemCacheDowngradedFileSegments` UInt64 COMMENT 'Number of file segments downgraded (moved) from the protected to the probationary queue in SLRU cache policy. This is an internal move within the cache, not an eviction.',
    ...
    `ProfileEvent_FilesystemCacheOvercommitCandidatesIterationSteps` UInt64 COMMENT 'Number of outer iterations the overcommit eviction loop ran beyond its first pass to satisfy a reservation\'s deficit',
    ...
    `ProfileEvent_FilesystemCacheLockOriginPoolMicroseconds` UInt64 COMMENT 'Lock filesystem cache shared origin pool time',
    ...
    `ProfileEvent_FilesystemCacheFailedReserveAttempts` UInt64 COMMENT 'Number of failed file cache space reservation attempts.',
    ...
    `ProfileEvent_FilesystemCacheIdleClientEvictions` UInt64 COMMENT 'Number of times all cache entries of an idle distributed-cache client (`user_id`) were purged because no access happened within `idle_client_ttl_sec`.',
    `ProfileEvent_FilesystemCacheBackgroundRemovedInvalidatedEntries` UInt64 COMMENT 'Number of invalidated (lazily-removed) priority queue entries purged by the background cleanup thread',
    `ProfileEvent_FilesystemCacheClientsMapLockWaitMicroseconds` UInt64 COMMENT 'Time spent waiting on a contended per-client (`user_id`) usage map shard lock in the overcommit cache policy',
    `ProfileEvent_FilesystemCacheInvalidatedEntriesCleanupThreadWorkMilliseconds` UInt64 COMMENT 'Time for which the background thread executed the invalidated priority queue entries cleanup job',
    ...
    `ProfileEvent_FilesystemCacheFreeSpaceKeepingThreadErrors` UInt64 COMMENT 'Number of exceptions caught by the background free space keeping job (collection, removal or finalization)',
    ...
    `ProfileEvent_AggregationConvertedToTwoLevel` UInt64 COMMENT 'How many times a single-level aggregation hash table was converted to two-level at runtime.',
    ...
    `ProfileEvent_KeeperBatchMaxCount` UInt64 COMMENT 'Number of batches that hit max_requests_batch_size limit',
    `ProfileEvent_KeeperBatchMaxTotalSize` UInt64 COMMENT 'Number of batches that hit max_requests_batch_bytes_size limit',
    `ProfileEvent_KeeperBatchMaxReadCount` UInt64 COMMENT 'Number of batches that hit max_read_batch_size limit',
    `ProfileEvent_KeeperBatchMaxReadTotalSize` UInt64 COMMENT 'Number of batches that hit max_read_batch_bytes_size limit',
    ...
    `ProfileEvent_KeeperWriteBatchCount` UInt64 COMMENT 'Number of write request batches processed by Keeper',
    `ProfileEvent_KeeperWriteBatchTotalRequests` UInt64 COMMENT 'Total number of write requests processed in batches by Keeper',
    ...
    `ProfileEvent_KeeperTTLRemoveRequestsEnqueued` UInt64 COMMENT 'Number of TTL remove requests successfully enqueued',
    ...
    `ProfileEvent_KeeperWatchesTriggered` UInt64 COMMENT 'Number of watch triggers',
    `ProfileEvent_KeeperWatchTriggeredNodeCreated` UInt64 COMMENT 'Number of watch triggers by `CREATE` operation',
    `ProfileEvent_KeeperWatchTriggeredNodeDeleted` UInt64 COMMENT 'Number of watch triggers by `DELETE` operation',
    `ProfileEvent_KeeperWatchTriggeredNodeDataChanged` UInt64 COMMENT 'Number of watch triggers by change operations',
    `ProfileEvent_KeeperWatchTriggeredNodeChildrenChanged` UInt64 COMMENT 'Number of watch triggers by children change operations',
    ...
    `ProfileEvent_ObjectStorageQueueBucketLockLostOwnership` UInt64 COMMENT 'Number of times ownership of a bucket lock was detected as lost in S3(Azure)Queue. Non-zero value indicates too small persistent_processing_node_ttl_seconds or a bug',
    `ProfileEvent_ObjectStorageQueueBucketLockRefreshes` UInt64 COMMENT 'Number of successful bucket lock refreshes in S3(Azure)Queue',
    ...
    `ProfileEvent_DistrCacheReadRequestRetries` UInt64 COMMENT 'Distributed Cache read buffer event. The number of read request retries after a retriable error (including retries of the initial request creation)',
    ...
    `ProfileEvent_DistrCacheReadErrors` UInt64 COMMENT 'Distributed Cache client event. Total number of distributed cache errors during read requests (equals DistrCacheReadErrorsRetriable + DistrCacheReadErrorsNonRetriable)',
    `ProfileEvent_DistrCacheReadErrorsRetriable` UInt64 COMMENT 'Distributed Cache client event. Number of retriable distributed cache errors during read requests (counted on each occurrence, including the ones recovered by a retry)',
    `ProfileEvent_DistrCacheReadErrorsNonRetriable` UInt64 COMMENT 'Distributed Cache client event. Number of non-retriable distributed cache errors during read requests',
    `ProfileEvent_DistrCacheReadRequestRetriesExhausted` UInt64 COMMENT 'Distributed Cache read buffer event. Number of times a retriable read error did not succeed within the read request retries limit',
    `ProfileEvent_DistrCacheWriteErrors` UInt64 COMMENT 'Distributed Cache client event. Total number of distributed cache errors during write requests (equals DistrCacheWriteErrorsRetriable + DistrCacheWriteErrorsNonRetriable). A server-initiated write-through cache `Stop` is flow control, not an error, and is not counted',
    `ProfileEvent_DistrCacheWriteErrorsRetriable` UInt64 COMMENT 'Distributed Cache client event. Number of retriable distributed cache errors during write requests (counted on each occurrence, including the ones recovered by a retry)',
    `ProfileEvent_DistrCacheWriteErrorsNonRetriable` UInt64 COMMENT 'Distributed Cache client event. Number of non-retriable distributed cache errors during write requests',
    `ProfileEvent_DistrCacheWriteRequestRetriesExhausted` UInt64 COMMENT 'Distributed Cache write buffer event. Number of times a retriable write error did not succeed within the write request retries limit',
    `ProfileEvent_DistrCacheWriteRequestRetries` UInt64 COMMENT 'Distributed Cache write buffer event. The number of write request retries after a retriable error',
    ...
    `ProfileEvent_DistrCacheBackgroundWrites` UInt64 COMMENT 'Distributed Cache write buffer event. Number of cache writes that ran on a background thread (a background write slot was acquired)',
    `ProfileEvent_DistrCacheInlineWrites` UInt64 COMMENT 'Distributed Cache write buffer event. Number of cache writes that ran inline on the calling thread (no background write slot was free, or the write was for temporary data)',
    ...
    `ProfileEvent_DistrCacheServerShutdownConnectionsMicroseconds` UInt64 COMMENT 'Distributed Cache server event. Time spent in shutdownAllConnections waiting for active connections to finish (or be forcefully closed) during server shutdown',
    ...
    `ProfileEvent_DistributedPlanRemoteTasks` UInt64 COMMENT 'Number of tasks dispatched to remote workers when executing a query with make_distributed_plan. A non-zero value means the query was actually executed distributedly.',
    `ProfileEvent_DistributedPlanLocalExecution` UInt64 COMMENT 'Set to 1 when a make_distributed_plan query was executed in-process via the local executor (distributed_plan_execute_locally) instead of being dispatched to remote workers.',
    `ProfileEvent_DistributedPlanHostsUsed` UInt64 COMMENT 'Number of distinct hosts that were assigned at least one task when executing a query with make_distributed_plan.',
    ...
    `ProfileEvent_SharedMergeTreePartitionOperationRetries` UInt64 COMMENT 'How many times a partition-level operation (DROP/MOVE/REPLACE/CLONE/TRUNCATE PARTITION) had to retry the Keeper multi-write because of a concurrent change',
    ...
    `ProfileEvent_ConcurrencyControlSlotsDelayed` UInt64 COMMENT 'Number of CPU slots not granted at allocation time due to capacity pressure. Excludes slots held back by lazy allocation.',
    ...
    `ProfileEvent_ConcurrencyControlQueriesDelayed` UInt64 COMMENT 'Number of queries delayed by CPU slot capacity pressure. Contention indicator — excludes lazy-upscale waits on an uncontested server.',
    ...
    `ProfileEvent_MemoryReservationAdmitMicroseconds` UInt64 COMMENT 'Total time a query was waiting for initial memory reservation to be admitted',
    `ProfileEvent_MemoryReservationIncreaseMicroseconds` UInt64 COMMENT 'Total time a query was waiting for memory reservation increase to be approved',
    `ProfileEvent_MemoryReservationIncreases` UInt64 COMMENT 'Total number of approved memory reservation increase requests',
    `ProfileEvent_MemoryReservationDecreases` UInt64 COMMENT 'Total number of approved memory reservation decrease requests',
    `ProfileEvent_MemoryReservationKilled` UInt64 COMMENT 'Total number of memory reservation kill signals',
    `ProfileEvent_MemoryReservationFailed` UInt64 COMMENT 'Total number of failed memory reservations',
    ...
    `ProfileEvent_SharedCatalogStateFetchFullFallback` UInt64 COMMENT 'Number of times the incremental fetch of the Shared Catalog state failed too many times in a row and a full state fetch was performed instead',
    ...
    `ProfileEvent_ExecutableUserDefinedFunctionInvocations` UInt64 COMMENT 'Number of executable and executable_pool user-defined function invocations.',
    `ProfileEvent_ExecutableUserDefinedFunctionElapsedMicroseconds` UInt64 COMMENT 'Wall clock time spent executing executable and executable_pool user-defined functions, in microseconds.',
    `ProfileEvent_ExecutableUserDefinedFunctionUserTimeMicroseconds` UInt64 COMMENT 'User mode CPU time consumed by executable and executable_pool user-defined function child processes, in microseconds.',
    `ProfileEvent_ExecutableUserDefinedFunctionSystemTimeMicroseconds` UInt64 COMMENT 'Kernel mode CPU time consumed by executable and executable_pool user-defined function child processes, in microseconds.',
    `ProfileEvent_ExecutableUserDefinedFunctionPeakMemoryByteSeconds` UInt64 COMMENT 'Per-process peak memory used by executable and executable_pool user-defined function child processes integrated over wall clock time, in byte-seconds.',
    `ProfileEvent_ExecutableUserDefinedFunctionInputBytes` UInt64 COMMENT 'Total bytes written to the stdin of executable and executable_pool user-defined function child processes.',
    `ProfileEvent_ExecutableUserDefinedFunctionOutputBytes` UInt64 COMMENT 'Total bytes read from the stdout of executable and executable_pool user-defined function child processes.',
    `ProfileEvent_ExecutableUserDefinedFunctionPoolWaitMicroseconds` UInt64 COMMENT 'Time spent waiting on tryBorrowObject when the executable_pool user-defined function pool is exhausted, in microseconds.',
    ...
    `ProfileEvent_IndexGenericExclusionSearchStepLimitReached` UInt64 COMMENT 'Number of times the generic exclusion search over the index marks reached merge_tree_generic_exclusion_search_max_steps and accepted the remaining mark ranges without further splitting',
    ...
    `ProfileEvent_RuntimeFilterGranulesConsidered` UInt64 COMMENT 'Number of granules examined for read time pruning by JOIN Runtime Filters',
    `ProfileEvent_RuntimeFilterGranulesDropped` UInt64 COMMENT 'Number of granules pruned at read time by JOIN Runtime Filters',
    ...
    `ProfileEvent_StatelessWorkerRequested` UInt64 COMMENT 'Number of stateless workers requested by queries for distributed query execution.',
    `ProfileEvent_StatelessWorkerProvided` UInt64 COMMENT 'Number of stateless workers provided to queries for distributed query execution.',
    `ProfileEvent_StatelessWorkerProvisioningMicroseconds` UInt64 COMMENT 'Total time queries spent waiting for stateless workers to be provisioned.',
    `ProfileEvent_StatelessWorkerProvisioningWaits` UInt64 COMMENT 'Number of times a query had to wait for the stateless worker discovery service to provide workers.',
    `ProfileEvent_StatelessWorkerCreateLeaseRequests` UInt64 COMMENT 'Number of create_lease requests sent to the stateless worker discovery service to acquire workers.',
    `ProfileEvent_StatelessWorkerCreateLeaseMicroseconds` UInt64 COMMENT 'Total time spent in create_lease requests to the stateless worker discovery service.',
    `ProfileEvent_StatelessWorkerCreateLeaseErrors` UInt64 COMMENT 'Number of failed create_lease requests to the stateless worker discovery service (any error or timeout).',
    `ProfileEvent_StatelessWorkerGetLeaseRequests` UInt64 COMMENT 'Number of get_lease requests sent to the stateless worker discovery service to refresh the list of leased workers.',
    `ProfileEvent_StatelessWorkerGetLeaseMicroseconds` UInt64 COMMENT 'Total time spent in get_lease requests to the stateless worker discovery service.',
    `ProfileEvent_StatelessWorkerGetLeaseErrors` UInt64 COMMENT 'Number of failed get_lease requests to the stateless worker discovery service (any error or timeout).',
    `ProfileEvent_StatelessWorkerUpdateLeaseRequests` UInt64 COMMENT 'Number of update_lease requests sent to the stateless worker discovery service to renew worker leases.',
    `ProfileEvent_StatelessWorkerUpdateLeaseMicroseconds` UInt64 COMMENT 'Total time spent in update_lease requests to the stateless worker discovery service.',
    `ProfileEvent_StatelessWorkerUpdateLeaseErrors` UInt64 COMMENT 'Number of failed update_lease requests to the stateless worker discovery service (any error or timeout).',
    ...
    `CurrentMetric_BackgroundStreamingSchedulePoolTask` Int64 COMMENT 'Number of active tasks in BackgroundStreamingSchedulePool for streaming queries tasks',
    `CurrentMetric_BackgroundStreamingSchedulePoolSize` Int64 COMMENT 'Limit on number of tasks in BackgroundStreamingSchedulePool for streaming queries tasks',
    ...
    `CurrentMetric_ReaderExecutorActive` Int64 COMMENT 'Number of live ReaderExecutor instances.',
    `CurrentMetric_ReaderExecutorChainedBufferBytes` Int64 COMMENT 'Bytes currently held in live ReaderExecutor chain buffers (OwnedChainedBuffer) -- live in-flight read memory, not a cumulative total.',
    `CurrentMetric_ReaderExecutorLongConnections` Int64 COMMENT 'Number of long source connections currently held open by ReaderExecutor for sequential read optimization.',
    ...
    `CurrentMetric_QueryRunnerThreads` Int64 COMMENT 'Number of threads in the QueryRunner thread pool.',
    `CurrentMetric_QueryRunnerThreadsActive` Int64 COMMENT 'Number of threads in the QueryRunner thread pool running a task.',
    `CurrentMetric_QueryRunnerThreadsScheduled` Int64 COMMENT 'Number of queued or active jobs in the QueryRunner thread pool.',
    ...
    `CurrentMetric_IcebergCompactionMerge` Int64 COMMENT 'Number of executing background Iceberg compaction merges.',
    `CurrentMetric_IcebergCompactionSourceFiles` Int64 COMMENT 'Number of input source files (data files and position delete files) participating in currently executing Iceberg compaction merges.',
    ...
    `CurrentMetric_UDFDriverInvokerThreads` Int64 COMMENT 'Number of threads in thread pools servicing pipes of executable UDF driver commands.',
    `CurrentMetric_UDFDriverInvokerThreadsActive` Int64 COMMENT 'Number of active threads in thread pools servicing pipes of executable UDF driver commands.',
    `CurrentMetric_UDFDriverInvokerThreadsScheduled` Int64 COMMENT 'Number of queued or active jobs in thread pools servicing pipes of executable UDF driver commands.',
    ...
    `CurrentMetric_FilesystemCachePriorityQueueElements` Int64 COMMENT 'Total number of entries in the filesystem cache priority queue, including invalidated entries pending removal',
    `CurrentMetric_FilesystemCacheInvalidatedElements` Int64 COMMENT 'Number of invalidated (zero size, pending removal) entries in the filesystem cache priority queue',
    ...
    `CurrentMetric_FilesystemCacheEvictionThreads` Int64 COMMENT 'Number of threads in the filesystem cache background eviction (removal) pool',
    `CurrentMetric_FilesystemCacheEvictionThreadsActive` Int64 COMMENT 'Number of threads in the filesystem cache background eviction pool running a task',
    `CurrentMetric_FilesystemCacheEvictionThreadsScheduled` Int64 COMMENT 'Number of queued or active jobs in the filesystem cache background eviction pool',
    ...
    `CurrentMetric_PaimonMetadataFilesCacheBytes` Int64 COMMENT 'Size of the Paimon metadata cache in bytes',
    `CurrentMetric_PaimonMetadataFilesCacheFiles` Int64 COMMENT 'Number of cached files in the Paimon metadata cache',
    ...
    `CurrentMetric_PointInPolygonCacheBytes` Int64 COMMENT 'Size in bytes of the cache of preprocessed constant polygons for the `pointInPolygon` function',
    `CurrentMetric_PointInPolygonCacheCells` Int64 COMMENT 'Number of preprocessed constant polygons in the cache for the `pointInPolygon` function',
    `CurrentMetric_PointInPolygonCacheSizeLimit` Int64 COMMENT 'Maximum size in bytes of the cache of preprocessed constant polygons for the `pointInPolygon` function (the applied value of the `point_in_polygon_cache_size` server setting)',
    ...
    `CurrentMetric_UniqueKeyIndexCacheBytes` Int64 COMMENT 'Total size of UNIQUE KEY index cache in bytes',
    `CurrentMetric_UniqueKeyIndexCacheEntries` Int64 COMMENT 'Total number of UNIQUE KEY index blocks cached',
    `CurrentMetric_DeleteBitmapCacheBytes` Int64 COMMENT 'Total size of the UNIQUE KEY delete-bitmap cache in bytes',
    `CurrentMetric_DeleteBitmapCacheEntries` Int64 COMMENT 'Total number of UNIQUE KEY delete bitmaps cached',
    ...
    `CurrentMetric_EncryptionHeaderCacheBytes` Int64 COMMENT 'Total size of the encryption header cache in bytes',
    `CurrentMetric_EncryptionHeaderCacheEntries` Int64 COMMENT 'Total number of entries in the encryption header cache',
    `CurrentMetric_CompiledExpressionCacheBytes` Int64 COMMENT 'Reserved page-block capacity (rounded up to whole pages with a 2x over-provisioning factor) held by `JITModuleMemoryManager` for executable/data sections of cached JIT-compiled functions. NOT the actual bytes of machine code in use (that\'s smaller). Allocated via `posix_memalign`, which is intercepted into jemalloc, so this is accounted within the dedicated JIT arena and is a subset of `jemalloc.jit_arena.active_bytes`.',
    `CurrentMetric_CompiledExpressionCacheCount` Int64 COMMENT 'Total entries in the cache of JIT-compiled machine code.',
    ...
    `CurrentMetric_KeeperTTLNodes` Int64 COMMENT 'Number of nodes with a TTL set currently stored in Keeper.',
    `CurrentMetric_KeeperContainerNodes` Int64 COMMENT 'Number of container nodes currently stored in Keeper.',
    ...
    `CurrentMetric_DiskObjectStorageCopyObjectThreads` Int64 COMMENT 'Number of threads in the thread pool used to parallelize copyObjectToAnotherObjectStorage calls inside object storage disk transactions',
    `CurrentMetric_DiskObjectStorageCopyObjectThreadsActive` Int64 COMMENT 'Number of threads in the disk object storage copy thread pool running a task',
    `CurrentMetric_DiskObjectStorageCopyObjectThreadsScheduled` Int64 COMMENT 'Number of queued or active tasks in the disk object storage copy thread pool',
    ...
    `CurrentMetric_MemoryReservationApproved` Int64 COMMENT 'Total number of currently allocated (approved) bytes for memory reservations',
    `CurrentMetric_MemoryReservationDemand` Int64 COMMENT 'Total number of outstanding increase request bytes for memory reservations',
    ...
    `CurrentMetric_StatelessWorkerInUse` Int64 COMMENT 'Number of stateless workers currently in use by running queries.',
    `CurrentMetric_StatelessWorkerIdle` Int64 COMMENT 'Number of stateless workers currently held by the server but not used by any running query.',
    `CurrentMetric_ExchangeServerThreads` Int64 COMMENT 'Number of threads in the distributed exchange server handshake thread pool.',
    `CurrentMetric_ExchangeServerThreadsActive` Int64 COMMENT 'Number of threads in the distributed exchange server handshake thread pool running a task.',
    `CurrentMetric_ExchangeServerThreadsScheduled` Int64 COMMENT 'Number of queued or active jobs in the distributed exchange server handshake thread pool.',
